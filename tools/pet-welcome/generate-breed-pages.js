...
const COST_MODELS = {
  dog: {
    '超小型犬': { years: 14, annual: [24, 38], reserve: [80, 180], initial: [15, 40] },
    '小型犬': { years: 14, annual: [28, 42], reserve: [80, 180], initial: [15, 40] },
    '中型犬': { years: 13, annual: [32, 48], reserve: [100, 220], initial: [18, 45] },
    '大型犬': { years: 11, annual: [38, 58], reserve: [120, 280], initial: [20, 50] }
  },
  cat: {
    '小柄な猫': { years: 15, annual: [16, 24], reserve: [60, 150], initial: [12, 30] },
    '標準的な猫': { years: 15, annual: [18, 28], reserve: [60, 150], initial: [12, 30] },
    '大型の猫': { years: 14, annual: [22, 34], reserve: [80, 180], initial: [15, 35] }
  }
};
const STARTER_PRODUCTS = {
  dog: [
    ['ペットケージ / クレート', '落ち着いて休める場所として最初に用意。成長後の体格に合うサイズを選びます。', 'ペットケージ 犬'],
    ['犬用トイレトレー', '迎えた初日から必要。成長後も無理なく方向転換できる大きさを選びます。', '犬 トイレトレー'],
    ['フードボウル・食器スタンド', '首や関節への負担を抑えるため、体高に合った高さを確認します。', '犬 食器 スタンド'],
    ['キャリーバッグ・キャリーケース', '通院や災害時の移動に必須。成長後の体格と耐荷重を確認します。', '犬 キャリーケース']
  ],
  cat: [
    ['ペットケージ / クレート', '落ち着いて休める場所として最初に用意。上下移動と休息ができる大きさを選びます。', '猫 ケージ'],
    ['猫用トイレ本体', '迎えた初日から必要。頭数＋1個を置ける場所も検討します。', '猫用トイレ 本体'],
    ['フードボウル・食器スタンド', 'ひげが当たりにくく、安定して洗いやすい器を選びます。', '猫 食器 スタンド'],
    ['キャリーバッグ・キャリーケース', '通院や避難に必須。上からも出し入れできる形は受診時に便利です。', '猫 キャリーケース']
  ]
};
function storeLinks(keyword) {
  const amazon = `https://www.amazon.co.jp/s?k=${encodeURIComponent(keyword)}&tag=yoshiwebnet-22`;
  const rakutenTarget = `https://search.rakuten.co.jp/search/mall/${encodeURIComponent(keyword)}/`;
  const rakuten = `https://hb.afl.rakuten.co.jp/hgc/${encodeURIComponent('0a659c4f.afcf3049.0a659c50.7d029f67')}/?pc=${encodeURIComponent(rakutenTarget)}`;
  const mercari = `https://jp.mercari.com/search?keyword=${encodeURIComponent(keyword)}`;
  return `<div class="store-links"><a class="amazon" href="${amazon}" target="_blank" rel="nofollow sponsored noopener">Amazonで探す</a><a class="rakuten" href="${rakuten}" target="_blank" rel="nofollow sponsored noopener">楽天で探す</a><a class="mercari" href="${mercari}" target="_blank" rel="nofollow sponsored noopener">メルカリで探す</a></div>`;
}
function requiredSections(species, item) {
  const kind = species === 'dog' ? '犬' : '猫';
  const model = COST_MODELS[species][item.size];
  const lifetime = [model.initial[0] + model.annual[0] * model.years + model.reserve[0], model.initial[1] + model.annual[1] * model.years + model.reserve[1]];
  const procedureThird = species === 'dog'
    ? `<a class="procedure-item amber" href="https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/kenkou_iryou/kenkou/kekkaku-kansenshou18/kyokenbyou.html" target="_blank" rel="noopener noreferrer"><strong>犬の登録・狂犬病予防注射 <span aria-hidden="true">↗</span></strong><span>犬の登録、年1回の予防注射、鑑札・注射済票について確認します。</span><b>厚生労働省の案内</b></a>`
    : `<a class="procedure-item amber" href="https://www.env.go.jp/nature/dobutsu/aigo/2_data/pamph/h2202.pdf" target="_blank" rel="noopener noreferrer"><strong>室内飼育と地域のルール <span aria-hidden="true">↗</span></strong><span>脱走防止、繁殖制限、所有者明示など、地域で暮らすための基本を確認します。</span><b>環境省の飼育ガイド</b></a>`;
  const products = STARTER_PRODUCTS[species].map(([name, note, keyword]) => `<article class="product-item"><h3>${name}</h3><p>${note}</p>${storeLinks(keyword)}</article>`).join('');
  return `<section class="feature-section procedures" aria-labelledby="procedures-title"><header class="feature-head"><p class="section-kicker indigo">PROCEDURES</p><h2 id="procedures-title">必要な手続きと確認先</h2><p>${kind}、迎え先、自治体によって必要な手続きは異なります。${item.breed}を迎える前に、対象になるものを一つずつ確認してください。</p></header><div class="procedure-grid"><a class="procedure-item indigo" href="https://www.env.go.jp/nature/dobutsu/aigo/pickup/owner.html" target="_blank" rel="noopener noreferrer"><strong>迎える前の基本 <span aria-hidden="true">↗</span></strong><span>終生飼養、迎え先、販売業者の確認ポイントを確認できます。</span><b>環境省の案内を見る</b></a><a class="procedure-item emerald" href="https://www.env.go.jp/nature/dobutsu/aigo/pickup/chip.html" target="_blank" rel="noopener noreferrer"><strong>マイクロチップの変更登録 <span aria-hidden="true">↗</span></strong><span>登録証明書を受け取り、所有者情報を新しい飼い主へ変更します。</span><b>環境省の手続き案内</b></a>${procedureThird}<a class="procedure-item slate" href="https://www.env.go.jp/nature/dobutsu/aigo/disaster.html" target="_blank" rel="noopener noreferrer"><strong>災害時の備え <span aria-hidden="true">↗</span></strong><span>避難先、同行避難、普段からの健康管理と備蓄を確認します。</span><b>環境省の防災情報</b></a></div><p class="section-note">マイクロチップの変更登録や自治体での手続きは、迎え先・登録状況・お住まいの市区町村によって扱いが異なる場合があります。必要書類と期限は各窓口へ確認してください。</p></section><section class="feature-section costs" aria-labelledby="costs-title"><header class="feature-head"><p class="section-kicker green">LIFETIME COST GUIDE</p><h2 id="costs-title">将来かかる費用の目安</h2><p>${item.breed}（${item.size}）の体格区分を反映し、迎え入れ後の費用を幅で試算します。購入・譲渡費用や個別の医療費は別途確認してください。</p></header><div class="cost-summary"><strong>${item.breed}（${item.size}）の試算</strong><span>試算年数：約${model.years}年</span></div><div class="cost-grid"><article><span>初期費用</span><strong>${model.initial[0]}万円〜${model.initial[1]}万円</strong><small>用品・初回医療</small></article><article><span>年間費用</span><strong>${model.annual[0]}万円〜${model.annual[1]}万円</strong><small>食事・予防・生活</small></article><article class="reserve"><span>医療・介護の備え</span><strong>${model.reserve[0]}万円〜${model.reserve[1]}万円</strong><small>別枠で確保する目安</small></article><article class="lifetime"><span>生涯費用の目安</span><strong>${lifetime[0]}万円〜${lifetime[1]}万円</strong><small>初期＋年間×年数＋備え</small></article></div><div class="cost-note"><h3>この試算をどう受け止めるか</h3><p>費用は、年齢・体格・健康状態・地域・暮らし方で変わります。毎年の支出だけでなく、急な治療、介護、災害、飼い主の入院や引っ越しにも備えましょう。</p><small>参考：<a href="https://mag.anicom-sompo.co.jp/30951" target="_blank" rel="noopener noreferrer">アニコム損保「2025年のペットにかけた年間支出調査」</a>。この試算は独自の参考モデルです。</small></div></section><section class="feature-section starter" aria-labelledby="starter-title"><header class="feature-head"><p class="section-kicker green">STARTER ITEMS</p><h2 id="starter-title">最初にそろえたいもの</h2><p>${item.breed}を迎えた最初の1週間に使いやすいものを優先しています。必要性と選ぶ基準を先に確認してください。</p></header><div class="product-grid"><div class="pr-notice"><strong><span>PR</span> アフィリエイト広告を利用しています</strong></div>${products}</div><p class="affiliate-note">当サイトはAmazonアソシエイト・プログラムおよび楽天アフィリエイトの参加者です。リンク先での購入により収益を得る場合があります。</p></section><section class="responsibility-section" aria-labelledby="responsibility-title"><div class="responsibility-icon" aria-hidden="true">♡</div><div><h2 id="responsibility-title">命を迎えるということ</h2><p>${item.breed}は、かわいい時期だけでなく、病気や老いも含めて家族として暮らす存在です。食事・運動・予防・医療・しつけ・環境づくりを続け、最後まで責任をもって世話をする覚悟があるかを、迎える前に家族で話し合いましょう。</p><ul><li>毎日の世話をする人と、代わりに支える人を決める</li><li>年間費用と、急な医療・介護に備える方法を考える</li><li>住まい・仕事・家族構成が変わっても暮らし続けられるか確認する</li></ul></div></section>`;
}
const FEATURE_CSS = `.feature-section{margin-top:20px;padding:28px;background:#fff;border:1px solid #e2e8f0;border-radius:22px;box-shadow:0 10px 28px #47556912}.feature-head{padding-bottom:18px;border-bottom:1px solid #e2e8f0}.feature-head h2{margin:4px 0 8px;font-size:24px;line-height:1.45;color:#1e293b}.feature-head>p:last-child{margin:0;color:#64748b;font-size:14px;line-height:1.9}.section-kicker{margin:0;font-size:12px;font-weight:800;letter-spacing:.14em}.section-kicker.indigo{color:#4338ca}.section-kicker.green{color:#047857}.procedure-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin-top:20px}.procedure-item{display:flex;flex-direction:column;min-height:150px;padding:18px;border:1px solid;border-radius:15px;text-decoration:none}.procedure-item strong{display:flex;justify-content:space-between;gap:12px;color:#1e293b;font-size:17px}.procedure-item span{margin-top:8px;color:#475569;font-size:14px;line-height:1.8}.procedure-item b{margin-top:auto;padding-top:12px;color:#4338ca;font-size:13px;text-decoration:underline;text-underline-offset:3px}.procedure-item.indigo{background:#eef2ff;border-color:#c7d2fe}.procedure-item.emerald{background:#ecfdf5;border-color:#a7f3d0}.procedure-item.amber{background:#fffbeb;border-color:#fde68a}.procedure-item.slate{background:#f8fafc;border-color:#cbd5e1}.procedure-item:focus-visible,.store-links a:focus-visible{outline:3px solid #a5b4fc;outline-offset:2px}.section-note,.affiliate-note{margin:20px 0 0;padding-top:16px;border-top:1px solid #e2e8f0;color:#64748b;font-size:12px;line-height:1.8}.cost-summary{display:flex;justify-content:space-between;gap:16px;margin-top:20px;color:#334155;font-size:14px}.cost-summary span{color:#64748b}.cost-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-top:14px}.cost-grid article{padding:18px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:15px}.cost-grid span,.cost-grid small{display:block;color:#64748b;font-size:12px;font-weight:700}.cost-grid strong{display:block;margin:8px 0 4px;color:#1e293b;font-size:19px;line-height:1.4}.cost-grid .reserve{background:#fffbeb;border-color:#fde68a}.cost-grid .reserve span,.cost-grid .reserve small{color:#92400e}.cost-grid .reserve strong{color:#78350f}.cost-grid .lifetime{background:#ecfdf5;border-color:#6ee7b7}.cost-grid .lifetime span,.cost-grid .lifetime small{color:#047857}.cost-grid .lifetime strong{color:#064e3b}.cost-note{margin-top:18px;padding:18px;background:#ecfdf5;border:1px solid #a7f3d0;border-radius:15px;color:#064e3b}.cost-note h3{margin:0 0 6px;font-size:16px}.cost-note p{margin:0;font-size:14px;line-height:1.8}.cost-note small{display:block;margin-top:8px}.cost-note a{color:#4338ca;font-weight:700}.product-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin-top:20px}.pr-notice,.product-item{padding:18px;border-radius:15px}.pr-notice{display:flex;align-items:center;min-height:120px;background:#fffbeb;border:1px solid #fcd34d;color:#92400e}.pr-notice span{display:inline-block;margin-right:8px;padding:2px 7px;border-radius:6px;background:#fde68a}.product-item{background:#f8fafc;border:1px solid #cbd5e1}.product-item h3{margin:0;color:#1e293b;font-size:17px}.product-item p{margin:6px 0 0;color:#64748b;font-size:14px;line-height:1.7}.store-links{display:flex;flex-wrap:wrap;gap:8px;margin-top:14px}.store-links a{padding:8px 11px;background:#fff;border:1px solid;border-radius:9px;font-size:13px;font-weight:700;text-decoration:none}.store-links .amazon{color:#4f46e5;border-color:#c7d2fe}.store-links .rakuten{color:#e11d48;border-color:#fecdd3}.store-links .mercari{color:#047857;border-color:#a7f3d0}.responsibility-section{display:flex;gap:16px;margin-top:20px;padding:28px;background:#1e293b;border:1px solid #334155;border-radius:22px;box-shadow:0 10px 28px #47556912;color:#cbd5e1}.responsibility-icon{flex:0 0 auto;color:#fcd34d;font-size:28px}.responsibility-section h2{margin:0;color:#f8fafc;font-size:22px}.responsibility-section p{margin:8px 0 0;font-size:14px;line-height:1.9}.responsibility-section ul{margin:12px 0 0;padding-left:1.2em;font-size:14px;line-height:1.9}@media(max-width:700px){.feature-section,.responsibility-section{padding:20px}.procedure-grid,.product-grid,.cost-grid{grid-template-columns:1fr}.cost-summary{display:block}.cost-summary span{display:block;margin-top:4px}.procedure-item{min-height:0}.store-links a{flex:1;text-align:center}}`;
...
for (const species of ['dog', 'cat']) {
  flatten(species).forEach((item, index) => {
    ...
    const html = page(species, item, index + 1)
      .replace('</style></head>', `</style><style>${FEATURE_CSS}@media(max-width:700px){.store-links a{padding:8px 4px;font-size:12px;white-space:nowrap}}</style></head>`)
